...
class OpenAICompatibleLLM {
  ...
  async chatJSON(systemPrompt, userPayload) {
    ...
    try { return JSON.parse(result); }
      ...
      if (first !== -1 && last > first) {
        try { return JSON.parse(result.slice(first, last + 1)); } catch {}
      }
      return null;
    }
  }
...
class VolcengineLLM extends OpenAICompatibleLLM {
  constructor(config) {
    super();
    this.config = config;
    this.model = config.model || "deepseek-v4-pro";
  ...
  async _getClient() {
    if (this._client) return this._client;
    const apiKey = getApiKey(this.config);
    if (!apiKey) throw new Error(`Missing API key for Volcengine (set apiKey or ${this.config.apiKeyEnv || "VOLCENGINE_API_KEY"})`);
    const OpenAI = (await import("openai")).default;
    this._client = new OpenAI({ apiKey, baseURL: this.baseURL });
    return this._client;
  }
...
PROVIDERS.volcengine = VolcengineLLM;
