...
|------|------|
| 公里分段 | 逐公里配速 + 心率 |
| 心率漂移 | HR 前1/3 vs 后1/3 差值百分比 |
| 中点配速漂移 | 前半程 vs 后半程平均配速对比 |
| 心率分区 | Z1-Z6 时间占比（COROS 6 区标准） |
| 配速变异系数 | CV 值反映匀速控制能力 |
| 步频分析 | 均值/标准差/<170/170-180/>180 三段占比 |
| 海拔分析 | 累计爬升/下降、净海拔、平均海拔 |
| 每圈摘要 | 圈号/距离/时间/配速/平均HR/最大HR/步频/功率 |
| 最大心率 | FIT Session 记录的最大心率 |
| GPS 坐标 | 逐航点经纬度（用于路线图展示） |
| 腕表温度 | 平均腕温（视手表支持） |
...
- 最近训练深度复盘（配速/心率/步频/最大心率分析 + 亮点/改进方向）
- 逐秒配速 & 心率图表（基于 FIT 航点数据）
- 中点配速漂移标签（前半程 vs 后半程配速对比 + 稳定性评估）
- 分段详情表（每圈摘要：距离/时间/配速/平均HR/最大HR/步频/功率）
- GPS 运动路线地图（Leaflet.js 在线加载，FIT GPS 坐标绘制）
- 心率最大峰值显示（FIT Session 最大心率）
- 其他近期训练（近7天）
...
| 隐私 | 位置坐标在报告中展示为路线图（仅保留有训练时的 GPS 点，不保留日时间戳） |
